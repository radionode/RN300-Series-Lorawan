...
function Decoder(bytes, port) {
    ...
    if (head === 11) {
      ...
      pm10 = parseInt(readUInt16LE(data.slice(offset, offset + raw_size)), 10);
      if (pm10 >= 65535) pm10 = null;
      offset += raw_size;

      lux = parseInt(readUInt16LE(data.slice(offset, offset + raw_size)), 10);
      if (lux >= 65535) lux = null;
      offset += raw_size;

      hcho = parseInt(readUInt16LE(data.slice(offset, offset + raw_size)), 10);
      if (hcho >= 65535) hcho = null;
      offset += raw_size;

      co2 = parseInt(readUInt16LE(data.slice(offset, offset + raw_size)), 10);
      if (co2 >= 65535) co2 = null;
      offset += raw_size;

      co = parseInt(readUInt16LE(data.slice(offset, offset + raw_size)), 10);
      if (co >= 65535) co = null;
      offset += raw_size;

      temperature = parseInt(readInt16LE(data.slice(offset, offset + raw_size)), 10) / 100;
      if (temperature <= -9999.0) temperature = null;
      offset += raw_size;

      humidity = parseInt(readInt16LE(data.slice(offset, offset + raw_size)), 10) / 100;
